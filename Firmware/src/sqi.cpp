...
static uint16_t countJumps16(const uint16_t *x, uint16_t n, uint32_t thr)
{
  ...
  for (uint16_t i = 1; i < n; i++)
  {
    if (ABSDIFF((uint32_t)x[i], (uint32_t)x[i - 1]) >= thr)
      c++;
  }
  return c;
}
                         ...
                         uint32_t hi, uint32_t ceilV)
{
  ...
  if (v >= lo && v <= hi)
    return 100;
  if (v < lo)
    return (uint8_t)(100u * (v - floorV) / (lo - floorV));
...
void sqiEvaluate(const Batch *b, Sqi *q)
{
...

  {
    ...
    if (ir.dc < SQI_IR_DC_MIN)
      q->flags |= SQI_F_NO_FINGER;
    if (ir.dc > SQI_IR_DC_MAX ||
        q->clip_n > SQI_CLIP_MAX_N)
      q->flags |= SQI_F_SATURATED;
    ...
    if (q->ir_pi < SQI_IR_PI_MIN_PERMIL)
      q->flags |= SQI_F_FLAT;
    if (q->ir_pi > SQI_IR_PI_MAX_PERMIL)
      q->flags |= SQI_F_MOTION;

    // Lompatan IR, dua tingkat.
    const uint32_t irSoftPermil = b->max_n ? (uint32_t)q->ir_jump_n * 1000u / b->max_n : 0;
    if (ir.p2p && ir.jump >= irHard)
      q->flags |= SQI_F_MOTION; // keras
    if (irSoftPermil > SQI_JUMP_SOFT_PERMIL)
      q->flags |= SQI_F_MOTION; // lunak

    // Noise tersebar pada IR.
    if (q->ir_tort10 >= SQI_IR_TORT_MAX_X10)
      q->flags |= SQI_F_MOTION;

    // Kanal SON1303.
    const uint32_t pgSoftPermil = b->ppg_n ? (uint32_t)q->ppg_jump_n * 1000u / b->ppg_n : 0;
    if (pg.p2p < SQI_PPG_P2P_MIN || pg.p2p > SQI_PPG_P2P_MAX)
      q->flags |= SQI_F_PPG_BAD;
    if (pg.p2p && pg.jump >= pgHard)
      q->flags |= SQI_F_PPG_BAD;
    if (pgSoftPermil > SQI_JUMP_SOFT_PERMIL)
      q->flags |= SQI_F_PPG_BAD;
    if (q->ppg_tort10 >= SQI_PPG_TORT_MAX_X10)
      q->flags |= SQI_F_PPG_BAD;
