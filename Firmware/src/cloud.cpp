...
static void otaTrialOnPost(bool ok)
{
  ...
  if (millis() - s_trialStart > OTA_TRIAL_TIMEOUT_MS)
    otaTrialRevert();
}
...
static bool otaCheckPending()
{
  ...
  bool pending = false;
  if (!jsonFindBool(body, "pending", &pending) || !pending)
    return false;
...
static bool cloudPost(const char *body, size_t len)
{
  ...
  if (strlen(CLOUD_API_KEY) > 0)
    s_http.addHeader("Authorization", "Bearer " CLOUD_API_KEY);
  ...
  if (!ok)
  {
    ...
    Serial.printf("[HTTP] %d  %u byte  %lu ms\n", code, (unsigned)len,
...
void netTask(void *arg)
{
  ...
  for (;;)
  {
    ...
    Batch *b = nullptr;
    if (xQueueReceive(g_qFilled, &b, pdMS_TO_TICKS(1000)) != pdTRUE)
      continue;
