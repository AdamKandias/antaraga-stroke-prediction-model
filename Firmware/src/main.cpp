...
{
  if (g_state == ST_ERROR && s != ST_ERROR)
    return; // ST_ERROR bersifat sticky
  g_state = s;
}
...
#define LED_PWM_CH 0
#define LED_PWM_ATTACH()                                  \
  do                                                      \
  {                                                       \
    ledcSetup(LED_PWM_CH, LED_PWM_FREQ_HZ, LED_PWM_BITS); \
    ledcAttachPin(PIN_LED, LED_PWM_CH);                   \
  } while (0)
...
static uint32_t breathDuty(uint32_t elapsedMs)
{
  ...
  float g = lvl;
  for (uint8_t i = 1; i < LED_BREATH_GAMMA; i++)
    g *= lvl;
...
static void ledService()
{
  ...
  if (g_state != prev)
  { // ganti status -> mulai pola dari awal
    prev = g_state;
    step = 0;
    stepStart = now;
    phaseT0 = now;
...
static void printStats()
{
  ...
  Serial.printf(
      ...
      (unsigned long)(g_stats.batches_sent
                          ? g_stats.sqi_flagged * 100UL / g_stats.batches_sent
                          : 0),
...
void setup()
{
  ...
  while (!Serial && millis() - t0 < 2000)
    delay(10); // USB-CDC, jangan blok selamanya
