...
static bool adcInit()
{
  ...
  if (adc_oneshot_config_channel(s_adc, ADC_CH_PPG, &chCfg) != ESP_OK)
    return false;
  if (adc_oneshot_config_channel(s_adc, ADC_CH_BATT, &chCfg) != ESP_OK)
    return false;
...
static bool adcInit()
{
  if (adc1_config_width(ADC_WIDTH_BIT_12) != ESP_OK)
    return false;
  if (adc1_config_channel_atten(ADC_CH_PPG, ADC_ATTEN_DB_11) != ESP_OK)
    return false;
  if (adc1_config_channel_atten(ADC_CH_BATT, ADC_ATTEN_DB_11) != ESP_OK)
    return false;
...
static uint8_t maxRead(uint8_t reg)
{
  ...
  if (Wire.endTransmission(false) != 0)
    return 0;
  if (Wire.requestFrom((int)MAX_ADDR, 1) < 1)
    return 0;
  return Wire.read();
}
...
static bool maxReadPtrs(uint8_t &wr, uint8_t &ovf, uint8_t &rd)
{
  ...
  if (Wire.endTransmission(false) != 0)
    return false;
  if (Wire.requestFrom((int)MAX_ADDR, 3) < 3)
    return false;
  wr = Wire.read();
  ovf = Wire.read();
  rd = Wire.read();
...
static bool maxBegin()
{
  ...
  delay(50);
  uint32_t t0 = millis();
  while ((maxRead(REG_MODE_CONFIG) & MODE_RESET) && millis() - t0 < 500)
    delay(1);
...
static uint8_t maxReadFifo(uint8_t n, uint32_t *red, uint32_t *ir)
{
  ...
  while (n > 0)
  {
    ...
    if (Wire.endTransmission(false) != 0)
      break;
    if (Wire.requestFrom((int)MAX_ADDR, (int)(chunk * 6)) < chunk * 6)
      break;
...
{
  if (mv >= kLipoCurve[0].mv)
    return 100;
  if (mv <= kLipoCurve[kLipoN - 1].mv)
    return 0;
...
static void readBattery(bool seed)
{
  ...
  if (seed || s_battEma <= 0.0f)
    s_battEma = mv;
  else
    s_battEma += BATT_EMA_ALPHA * (mv - s_battEma);
...
static Batch *batchAcquire()
{
  ...
  if (xQueueReceive(g_qFree, &b, 0) != pdTRUE)
  {
    ...
    if (xQueueReceive(g_qFilled, &b, 0) == pdTRUE)
...
void sensorTask(void *arg)
{
  ...
  {
    vTaskDelayUntil(&lastWake, kPeriod);
    if ((TickType_t)(xTaskGetTickCount() - lastWake) >= kPeriod)
      g_stats.ppg_overrun++;
    ...
    if (b->ppg_n == 0)
    { // titik pertama = penanda waktu batch
      b->seq = seq;
      b->t0_ms = millis();
