...
#define INVERT_PPG  0
struct Biquad {
  float b0,b1,b2,a1,a2; float z1,z2;
...
static inline uint16_t adcOversample() {
  ...
  for (int i=0;i<OVERSAMPLE;i++)
    if (adc_oneshot_read(adcHandle, ADC_CHAN, &v)==ESP_OK) acc += (uint32_t)v;
...
void IRAM_ATTR onSampleTimer() {
  ...
  if (next != ringTail){ ringBuf[ringHead]=raw; ringHead=next; }
...
void setup() {
  ...
  Serial.print(OVERSAMPLE); Serial.println(F("x, tanpa sinkron-50Hz."));
  Serial.println(F("# Pastikan elektroda ground + low-pass RC terpasang, DIAM total."));
  Serial.print(F("# Warmup ")); Serial.print(WARMUP_SECONDS);
  Serial.print(F(" dtk, rekam ")); Serial.print(RECORD_SECONDS); Serial.println(F(" dtk."));
...
void loop() {
  ...
  while (ringTail != ringHead) {
    ...
    float f = hp.process(voltage_mV); f = lp.process(f);
    ...
    Serial.print(raw);       Serial.print(',');   // raw MENTAH (belum band-pass)
    Serial.println(sigv,4);

    if (++sampleIdx >= recordSamples){
      Serial.println(F("# END"));
      Serial.print(F("# Total sampel: ")); Serial.print(recordSamples);
      Serial.print(F("  @ ")); Serial.print(FS,0); Serial.print(F(" Hz, oversample="));
      Serial.print(OVERSAMPLE); Serial.println(F("x"));
