...
bool readSample(uint32_t &red, uint32_t &ir)
{
  ...
  Wire.endTransmission(false);
  if (Wire.requestFrom((int)MAX30102_ADDR, 6) < 6)
    return false;
  uint8_t b[6];
  for (int i = 0; i < 6; i++)
    b[i] = Wire.read();
...
void loop()
{
  ...
  for (uint8_t s = 0; s < n; s++)
  {
    ...
    if (++sampleIdx >= recordSamples)
    {
      ...
      Serial.print(FS, 0);
      Serial.println(F(" Hz"));
      Serial.print(F("# Saturasi RED: "));
      Serial.print(satRed);
      Serial.print(F("  IR: "));
      Serial.print(satIr);
      Serial.print(F("  | FIFO overflow (throughput): "));
      Serial.println(ovfTotal);
      if (satRed || satIr)
