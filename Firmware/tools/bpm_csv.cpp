...
static float ppgWave(float phase) {          // phase 0..1 dalam satu denyut
...
static float noise() {                        // seragam -1..1, deterministik
...
static void runSynth(const SynthCfg& c) {
  ...
  for (uint32_t i = 0; i < n; i++) {
    ...
    if (phase >= 1.0f) { phase -= 1.0f; beatsGen++; }
    ...
    float gain = 1.0f;
    if (c.dropEveryN > 0 && ((beatsGen + 1) % (uint32_t)c.dropEveryN) == 0) gain = 0.10f;

    const float wander = c.wanderLsb * sinf(2.0f * (float)M_PI * 0.25f * t);
    float v = c.dcLevel + wander + gain * c.acAmp * ppgWave(phase)
            + c.noiseLsb * noise();

    if (v < 0.0f)                 v = 0.0f;
    if (v > (float)PPG_ADC_MAX)   v = (float)PPG_ADC_MAX;
...
static void runStep(float bpm1, float bpm2, float seconds) {
  ...
  for (uint32_t i = 0; i < n; i++) {
    ...
    const float v = 1800.0f + 60.0f * sinf(2.0f * (float)M_PI * 0.25f * t)
                  + 200.0f * ppgWave(phase) + 4.0f * noise();
    bpmPush((uint16_t)(v + 0.5f), &o);
...
static int runCsv(const char* path) {
  FILE* f = fopen(path, "r");
  if (!f) { fprintf(stderr, "tidak bisa membuka %s\n", path); return 1; }
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    nS++;
    if (o.status == BPM_LOCKED) { nLocked++; sumBpm += o.bpm; }
  }
  fclose(f);
...
int main(int argc, char** argv) {
  if (argc > 1) return runCsv(argv[1]);
  ...
  for (unsigned i = 0; i < sizeof(tests) / sizeof(tests[0]); i++) runSynth(tests[i]);
