...
static uint16_t greenRead()
{
  ...
  if (ok != GREEN_OVERSAMPLE)
    s_adcFail += (uint32_t)(GREEN_OVERSAMPLE - ok);
...
{
  if (regReadOr(R_PART_ID, 0x00) != 0x15)
    return false;
  regWrite(R_MODE, MODE_RESET);
  delay(60);
  const uint32_t t0 = millis();
  while ((regReadOr(R_MODE, 0) & MODE_RESET) && millis() - t0 < 600)
    delay(2);
...
static void handleSet(const char *s)
{
  ...
  case 'f':
    ...
    break;
  case 'd':
    s_rge = (uint8_t)constrain(v, 0, 3);
    break;
  case 'a':
    s_ave = (uint8_t)constrain(v, 0, 5);
    break;
  case 's':
    s_sr = (uint8_t)constrain(v, 0, 7);
    break;
  case 'w':
    s_pw = (uint8_t)constrain(v, 0, 3);
    break;
  default:
    Serial.println(F("#err setel tidak dikenal"));
    return;
  }
  applyCfg();
  printCfg();
...
void setup()
{
  ...
  while (millis() - ts < GREEN_SETTLE_MS)
  {
    (void)greenRead();
    delay(2);
  }
