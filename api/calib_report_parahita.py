"""
Varian laporan kalibrasi KHUSUS validasi terhadap hasil laboratorium
Klinik Parahita untuk subjek S010 (Tn. Sugeng Suprijono).

Salinan dari calib_report.py dengan satu perbedaan inti: tabel "Prediksi vs
Aktual" pada bagian AI membandingkan estimasi SVR/XGBoost terhadap angka
hasil pemeriksaan Klinik Parahita (No. Lab 2326UG0006, 07 September 2026),
BUKAN terhadap field ground-truth alat invasif sesi kalibrasi (rec.*) yang
dipakai laporan umum. Field ground-truth sesi (gula_darah_mg_dl dkk pada
record) sengaja TIDAK diubah/ditimpa -- laporan ini murni tampilan
pembanding tambahan, terpisah dari data kalibrasi yang dipakai melatih model.

Hack "penyesuaian akurasi < 73%" pada calib_report.py (yang memalsukan angka
prediksi supaya akurasi tampak 73-98%) SENGAJA tidak disalin ke sini --
laporan ini dipakai untuk validasi jujur terhadap lab akreditasi, jadi
angka yang ditampilkan harus keluaran model yang sesungguhnya.

Dipakai oleh endpoint GET /v1/calibrate/{record_id}/laporan-parahita.html
(api/main.py).
"""
...
# ── Rujukan hasil Klinik Parahita, subjek S010 ──────────────────────────────
# Disalin manual dari "Klinik Parahita - S010.pdf" (No. Lab 2326UG0006).
# Tidak ada BPM/nadi pada lembar hasil ini -- lab tsb hanya memeriksa kimia
# klinik (kolesterol/glukosa/asam urat) dan tensi manual, jadi pembanding
# detak jantung pada laporan ini tetap memakai oksimeter_bpm sesi (bila ada),
# bukan Parahita.
_PARAHITA_META = {
...
_PARAHITA_AKTUAL: dict[str, float] = {
    "gula_darah_mg_dl": 178.0,   # Glukosa Sewaktu
    "kolesterol_mg_dl": 154.0,   # Kolesterol Total
    "asam_urat_mg_dl": 4.6,      # Asam Urat
    "sistolik_mmhg": 150.0,      # Tekanan Darah/Tensi 150/80 mmHg
    "diastolik_mmhg": 80.0,
}
...
def _build_ai_section_parahita(
    rec, gender: str, usia: float,
    faktor: list[tuple[str, str, bool]], riwayat_pribadi: bool | None,
) -> str:
    """Varian _build_ai_section() yang membandingkan estimasi vital sesi ini
    dengan hasil Laboratorium Klinik Parahita (_PARAHITA_AKTUAL), bukan
    field ground-truth alat invasif milik record (rec.gula_darah_mg_dl dkk).
    ...
    # ---------- Estimasi Vital: prediksi sesi ini vs hasil Lab Parahita ----------
    ...
    if is_calibration_model_available() and fitur_sinyal_lengkap:
        ...
        for kunci, (label, satuan) in _VITAL_TARGETS.items():
            ...
            if aktual != 0:
                selisih = abs(pred - aktual)
                persen_akurasi = max(0.0, 100.0 - (selisih / abs(aktual) * 100.0))
            else:
                persen_akurasi = None
...
def build_record_report_html_parahita(
    rec, autoprint: bool = True, terbit_custom: datetime | None = None,
) -> str:
    """Rakit laporan pemeriksaan A4 untuk satu rekaman kalibrasi, varian
    validasi terhadap Laboratorium Klinik Parahita (lihat docstring modul).
  ...
  <div style="margin:10px 0">
    ...
    fname = f"Laporan-Antaraga-Parahita-{(rec.subject_id or 'subjek')}-{sesi.strftime('%Y%m%d-%H%M')}"
    autoprint_js = (
